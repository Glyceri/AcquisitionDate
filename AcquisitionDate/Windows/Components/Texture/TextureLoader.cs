...
namespace AcquistionDate.PetNicknames.Windowing.Components.Texture;
...
internal static class TextureLoader
{
    ...
    internal static UldFile? LoadUldFile(string path)
    {
        ...
        {
            uldFile = PluginHandlers.DataManager.GetFile<UldFile>(path);
    ...
    internal static unsafe UldIcon? LoadUld(string uldPath, int partsId, int partId)
    {
        ...
        UldFile? uldFile = LoadUldFile(uldPath);
    ...
    internal static IDalamudTextureWrap? LoadTexture(string path)
    {
        if (!PathToTexFileCache.TryGetValue(path, out TexFile? texFile))
        {
            path = PluginHandlers.TextureSubstitutionProvider.GetSubstitutedPath(path);

            texFile = Path.IsPathRooted(path)
                ? PluginHandlers.DataManager.GameData.GetFileFromDisk<TexFile>(path)
                : PluginHandlers.DataManager.GetFile<TexFile>(path);
        ...
        return PluginHandlers.TextureProvider.CreateFromRaw(RawImageSpecification.Bgra32(texFile.Header.Width, texFile.Header.Height), texFile.ImageData.AsSpan());
