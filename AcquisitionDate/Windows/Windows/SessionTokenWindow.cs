...
internal class SessionTokenWindow : AcquisitionWindow
{
    ...
    protected override void OnDraw()
    {
        ...
        BasicLabel.Draw("4. Click the network tab in that window.", size);
        BasicLabel.Draw("5. Now click on the quest section on the lodestone.", size);
        BasicLabel.Draw(" * A LOT of stuff should now show in the Network tab.", size);
        BasicLabel.Draw("6. Scroll to the top of the list and click on 'quest/'.", size);
        ...
        BasicLabel.Draw("7. Click on the 'Cookies' tab.", size);
        BasicLabel.Draw("8. Find the section that says: 'ldst_sess and copy the value.", size);
        ...
        BasicLabel.Draw("9. Now paste the session token into the field 'Session Token' in the 'Acquisition Window'", size);
        BasicLabel.Draw("10. After you are done downloading LOG OUT of the lodestone.\nDon't just click it away LOG OUT! This invalidates your session token.", new Vector2(ImGui.GetContentRegionAvail().X, BarSize * 2));
